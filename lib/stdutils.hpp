...
//Objects
crypto crypto; //crypto by crypto
...
public:
	...
	* windows libraries */
	...
	/* Summary: This function is used to return corrently path of
	...
	/* Summary: This function is used to clear a unix/linux console with
	 *  system() function */
	...
	/* Summary: This function is used to use wget command in linux console */
	...
	/* Summary: This function is used to return Operation System name. It
	 * use OS Defenitions Compiler to detect this
	 * NOTE: Isn't recommended to use that */
	...
	/* Summary: This function return bool type (0/1) to confirm
	 * if domain name is valid or not.
	 * NOTE: This doesn't use sockets*/
    ...
    bool is_valid_domain_name(const string& str)
    {
        ...
        for(i=0,segsize=0; i<ix; i++)
        {
            if (s[i]=='.')
            {
                if (segsize==0) //e.g: fail for abc..com
                    return false;
                segsize=0;
            }
            else if ( ('0' <= s[i] && s[i] <='9')
